...
class ProductBase(BaseModel):
    ...
    price: Optional[Decimal] = Field(None, ge=0, decimal_places=2, example=85.50)
...
class ProductCreate(ProductBase):
    # Hacer category_id obligatorio al crear si es requerido por negocio
    category_id: int = Field(..., description="ID de la categoría a la que pertenece")

class ProductUpdate(BaseModel): # Actualización parcial
...
class ProductRead(ProductBase):
    ...
    # Incluir la categoría relacionada en la lectura
